...
{
    public partial class EndingContractAuditList : BasePage
    {
        #region viewstate
        /// <summary>
        /// 用于存储 状态的ViewState.
        ...
        private void BindGridData(string state, string searchText)
        {
            ...
            if ((!string.IsNullOrEmpty(state)) && (state.Equals("8") || state.Equals("11")))
            {
            ...
            if (!string.IsNullOrEmpty(state))
            {
                ...
                switch (state)
                {
                    ...
                    case "7":
                        ...
                        break;
                    case "8":
                        strCondtion.Append(" AND  (Status = 7 OR Status = 8)  ");
                        break;
                    case "9":
                        strCondtion.Append(" AND Status = 9 ");
                        break;
                    case "11":
                        strCondtion.Append(" AND Status = 11 ");
                        break;
                    default:
                        break;
                }
        ...
        protected void gridData_RowDataBound(object sender, GridRowEventArgs e)
        {
            ...
            if (!_Info.NextOperaterId.Equals(this.CurrentUser.ObjectId) || ddlstState.SelectedValue != "7")
            {
